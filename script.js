...
function updateLeaderboard(data) {
  ...
  const bestConversion = [...players].sort((a, b) => b.conversion - a.conversion)[0];
  ...
  body.innerHTML = players.map((p, i) => {
    ...
    if (bestConversion && p.name === bestConversion.name) badges += `<span class="badge conversion">🎯 Best Conv</span>`;
    if (i === 0) badges += `<span class="badge closer">🏆 Top Closer</span>`;
